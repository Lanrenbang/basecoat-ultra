  ...
  >
    <nav aria-label="Sidebar navigation" class="h-full flex flex-col w-64 bg-card border-r">
      <header class="p-4 border-b h-14 flex items-center">
        <a href="{base}/svelte" class="flex items-center gap-2 font-semibold text-lg">
              ...
              {#each category.items as item}
                <li>
                  {#if item.id === 'neumorphism-demo'}
                    <a href="{base}/svelte/neumorphism-demo" class="block px-2 py-1.5 rounded-md hover:bg-accent hover:text-accent-foreground text-sm">
                      {item.name}
                    </a>
                  {:else}
